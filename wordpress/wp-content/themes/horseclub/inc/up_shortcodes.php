...
if ($porcol == '1') { $columnnum = 'onecolumn'; $imgwidth = 1150; $imgheight = 643; $sliderheight = 643;}
				   ...
				   else {$columnnum = 'p4'; $imgwidth = 300; $imgheight = 300; $sliderheight = 300;}   
...
if(empty($gpbtn)){
$output .=' <a class="image-popup-no-margins up-button  port_but" title="DEFAULT BUTTON" href="'.$img_url.'"><span><i class="fa fa-expand"></i></span></a>';
...
$comments_count = get_comments_number();
				...
				$output .= '</div>';
$output .= '</div>';
$output .= '<div class="up_latest_post_ex">'.up_excerpt(20).'</div>';
$output .= '</div>';
$output .= '</div>';
...
wp_register_script('sensor', 'https://maps.google.com/maps/api/js?=false', array('jquery'), '1.0', true);
...
class WPBakeryShortCode_horseclub_service_box extends WPBakeryShortCode {
    protected function content($atts, $content = null) {
		...
		$output  = '<div class="ih-item square effect4"><a href="' . $servlink. '">';
		 $output .='<div class="img"><img src="' . $servimg. '" alt="Service" /></div>';
